...
hydra -L /users -V -P /passwords -t 2 $server ssh | tee -a $name.log 


echo 'ATTEMPTS:' $( grep ATTEMPT ${name}.log |wc -l) |tee -a $log_file ; echo "[ENDING  $(date +%D-%T) ]" | tee -a $log_file; exit 0;